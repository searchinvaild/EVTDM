...
Foam::viscosityModels::timeVaryingGrout::calcNu() const
{
    ...
    scalar timeIndex = U_.time().value(); // 获取当前时间索引

    dimensionedScalar tone("tone", dimTime, 1.0); // 定义时间单位
    dimensionedScalar rtone("rtone", dimless/dimTime, 1.0); // 定义无量纲时间单位

    tmp<volScalarField> sr = strainRate(); // 获取应变率场
    volScalarField srLimited = max(sr(), dimensionedScalar("VSMALL", dimless/dimTime, VSMALL)); // 限制应变率，避免除以零
    ...
    dimensionedScalar kEffective = min(nuMax_, k_*exp(timeCoeff_*timeIndex)); // 计算有效的真实粘度值，限制在 nuMax_ 范围内

     tmp<volScalarField> nuCalc = 
        (tau0_ + kEffective*rtone*pow(tone*sr(), n_)) / srLimited; // 计算粘度场
    ...
    if (alpha1Ptr)
    {
       ...
       // 定义 VSMALL 阈值
        ...
        // 创建 k*exp(timeCoeff*t) 的调试场
         volScalarField kExpField(
            IOobject
            (
                "kExpField",
                U_.time().timeName(),
        ...
        Debug3_.primitiveFieldRef() = mask.primitiveField() * kExpField.primitiveField();
        ...
        forAll(Debug1_.boundaryFieldRef(), patchi)
        {
            ...
            Debug3_.boundaryFieldRef()[patchi] = 
                mask.boundaryField()[patchi] * kExpField.boundaryField()[patchi];
        }
        ...
        // 输出调试信息
        Info<< "    Debug1 (strain rate) range: " << min(Debug1_).value() << " to " << max(Debug1_).value() << endl;
        Info<< "    Debug2 (nu) range: " << min(Debug2_).value() << " to " << max(Debug2_).value() << endl;
        Info<< "    Debug3 (k_effective) range: " << min(Debug3_).value() << " to " << max(Debug3_).value() << endl;
        Info<< "    k*exp(timeCoeff*t) = " << k_.value()*exp(timeCoeff_.value()*timeIndex) << endl;
        Info<< "    k_effective = min(nuMax, k*exp(timeCoeff*t)) = " << kEffective.value() << endl;
        Info<< "    Number of cells with alpha.grout > 0.9: " 
